...
void sendStatus()
{
  ...
  for (int i = 0; i < numLeds; i++)
  {
    ...
    led["color"] = ledConfig[i].color;
    led["overrideColor"] = ledConfig[i].overrideColor;
    led["isForced"] = ledConfig[i].isForced;
  }
...
/**
 * Send the status of a single led.
 */
void sendStatus(String ledName)
{
  int i;
  for (i = 0; i < numLeds; i++)
  {
    if (ledConfig[i].id == ledName)
    {
      break;
    }
  }
  ...
  doc.clear();
  doc["id"] = ledConfig[i].id;
  doc["color"] = ledConfig[i].color;
  doc["overrideColor"] = ledConfig[i].overrideColor;
  doc["isForced"] = ledConfig[i].isForced;
  Serial.print("OK: set ");
  serializeJson(doc, Serial);
  Serial.println();
  Serial.flush();
...
void loop()
{
  if (Serial.available() > 0)
  {
    ...
    if (incomingComand.startsWith("status"))
    {
      sendStatus();
    }
    else
